...
class WeChat(WeChatBase):
    ...
    def GetNextNewMessage(self, savepic=False, savefile=False, savevoice=False, timeout=10):
        """获取下一个新消息"""
        # 直接从MsgItems中提取消息ID，避免递归调用
        MsgItems = self.C_MsgList.GetChildren()
        msgids = [''.join([str(i) for i in item.GetRuntimeId()]) for item in MsgItems if item.ControlTypeName == 'ListItemControl']
    ...
    def GetListenMessage(self, who=None):
        ...
        Returns:
            str|dict: 如果指定了who，返回该聊天对象的新消息；否则返回所有监听对象的新消息
        """
        try:
            # 如果指定了聊天对象
    ...
    def RemoveListenChat(self, who):
        ...
        if who in self.listen:
            # 获取聊天窗口对象
            chat_wnd = self.listen[who]

            try:
                # 尝试关闭聊天窗口
                chat_hwnd = FindWindow(name=who, classname='ChatWnd')
                if chat_hwnd:
                    wxlog.debug(f"关闭聊天窗口: {who}")
                    win32gui.PostMessage(chat_hwnd, win32con.WM_CLOSE, 0, 0)
            except Exception as e:
                wxlog.error(f"关闭聊天窗口失败: {str(e)}")
